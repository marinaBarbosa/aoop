...
public class OffensiveStrategy implements Strategy {
	...
	@Override
		...
		int xy = size/2;
		if (b.get(xy, xy) == b.FREE) {
			return new Move(xy,xy);
		}
		if (b.get(0, 0) == b.FREE)
			return new Move(0,0);
		if (b.get(0, size-1) == b.FREE)
			return new Move(0,size-1);
		if(b.get(size-1, 0) == b.FREE)
			return new Move(size-1,0);
		if(b.get(size-1, size-1) == b.FREE)
			return new Move(size-1,size-1);
